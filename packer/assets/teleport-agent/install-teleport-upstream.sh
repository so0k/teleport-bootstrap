...
install_teleport() {
  ...
  *)
    ...
    *)
      ...
      echo "There is no officially supported package to your package manager. Downloading and installing Teleport via curl."
      install_via_curl
      ;;
    esac
    ;;
...
if [ $# -ge 2 ] && [ -n "$2" ]; then
  ...
  oss | "") ;;
  *)
    echo 'ERROR: The second parameter must be "oss", "team", "cloud", or "enterprise".'
    exit 1
    ;;
  esac
fi
